...
// ResilientReportRegistry extends the ReportRegistry with resilience capabilities
type ResilientReportRegistry struct {
	mu            sync.Mutex
	reports       map[string]*pb.ReportResponse
	doneReports   map[string]int
...
// NewResilientReportRegistry creates a new resilient report registry
func NewResilientReportRegistry(moduleName string) *ResilientReportRegistry {
	rr := &ResilientReportRegistry{
...
func (rr *ResilientReportRegistry) applyUpdate(reportState *ReportState, msgWindow *window.MessageWindow, updateArgs *ReportUpdateArgs) {
...
func (rr *ResilientReportRegistry) Dispose() {
	rr.stateHelper.Dispose()
}

// WaitForReport waits for a report to be ready and returns it
func (rr *ResilientReportRegistry) WaitForReport(clientID string) *pb.ReportResponse {
...
func (rr *ResilientReportRegistry) GetReport(clientID string) *pb.ReportResponse {
	rr.mu.Lock()
	defer rr.mu.Unlock()
...
func (rr *ResilientReportRegistry) DoneAnswer(clientID string) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	...
	updateArgs := ReportUpdateArgs{
		ClientID:     clientID,
		IsDoneMarker: true,
		MessageID:    generateMessageID(), // Implement a function to generate unique IDs
	}
...
func (rr *ResilientReportRegistry) AddToAnswer1(clientID string, entry *pb.MovieEntry) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
...
func (rr *ResilientReportRegistry) AddAnswer2(clientID string, answer2 *pb.Answer2) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
...
func (rr *ResilientReportRegistry) AddAnswer3(clientID string, answer3 *pb.Answer3) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
...
func (rr *ResilientReportRegistry) AddAnswer4(clientID string, answer4 *pb.Answer4) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
...
func (rr *ResilientReportRegistry) AddAnswer5(clientID string, answer5 *pb.Answer5) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
